    ...
    public ActionResult SAMLPost(string provider)
    {
        foreach (var config in SSOPlugin.Instance.Configuration.SamlConfigs)
        {
            if (config.SamlClientId == provider && config.Enabled)
            {
                ...
                return Content(WebResponse.SamlGenerator(xml: Convert.ToBase64String(System.Text.Encoding.UTF8.GetBytes(samlResponse.Xml)), provider: provider), MediaTypeNames.Text.Html);
            }
        }
            ...
            if (config.OIDClientId == provider && config.Enabled)
            {
                ...
                foreach (var claim in result.User.Claims)
                {
                    ...
                    if (claim.Type == "preferred_username")
                    {
                        ...
                        return Content(WebResponse.OIDGenerator(data: Request.Query["state"], provider: provider), MediaTypeNames.Text.Html);
                    }
                }
